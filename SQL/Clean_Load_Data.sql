...
SELECT CAST(TRANSACTIONID AS UNSIGNED) TRANSACTIONID,
       ...
       CASE
           ...
           ELSE TIME(DEPTIME)
       END DEPTIME,
       CAST(DEPDELAY AS SIGNED) DEPDELAY,
       CAST(TAXIOUT AS SIGNED) TAXIOUT,
       CASE
       ...
       CASE
           ...
           ELSE TIME(ARRTIME)
       END ARRTIME,
       CAST(ARRDELAY AS SIGNED) ARRDELAY,
...
SELECT 'transaction_id', 'flight_date', 'airline_code', 'tail_num', 'flight_num', 
       'orgin_airport', 'dest_airport', 'sched_deptime', 'dep_time', 'dep_delay',
       'taxi_out', 'wheels_off', 'wheels_on', 'taxi_in', 'sched_arr_time', 'arr_time',
       'arr_delay', 'sched_elasped_time', 'act_elasped_time', 'cancelled',
       'diverted', 'distance '
UNION
SELECT *     
FROM flights.flights
INTO OUTFILE 'C:/ProgramData/MySQL/MySQL Server 9.4/Uploads/flights.csv' 
FIELDS TERMINATED BY ',' ENCLOSED BY '"';    
